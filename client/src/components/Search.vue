...
<button v-if="searchVisibleCity || searchVisibleKitchen" @click.prevent="handleHideSearchDrops" class="fixed h-full w-full top-0 right-0 bottom-0 left-0 opacity-50 cursor-auto"></button>
...
<label>
       City
        <select v-model="cities" class="shadow block bg-yellow-100 border-gray-400 border-2 rounded-lg">
      ...
      <div   class="shadow block z-10">
        ...
        >
          ...
          <ul class="list-reset">
            ...
            <button class=" hover:bg-yellow-200" v-if="searchQuery.toLowerCase() === k.name.toLowerCase().substring(0,searchQuery.length)" @click.prevent="selectKitchen(k.name)">
              {{k.name}}
            </button>
            </li>
          </ul>
...
export default {
  ...
  let burners = ""
    ...
    const { result, onResult } = useQuery(getKitchens)
...
<style lang="scss" scoped>
      ...
      // @input.prevent="handleSearchVisibleKitchen"
