...
export default {
    setup() {
        ...
        let testVar = ref(
            'http://localhost:8080/80bcafeb-612b-4833-9fc9-fe974a3f6199'
        )
        ...
        const simpleCanvasTest = () => {
            const { canvas, coordinates, visibleArea } =
            ...
            axios({
                ...
                axios({
                    ...
                    .then((response) => {
                        ...
                        imageLoc.value = (URL || webkitURL).createObjectURL(
                            response.data
                        )
...
// const handleUploadFiles = (e) => { // debugger // // const fr = new
FileReader() // // var buffer = new ArrayBuffer(100); // // var view = new
DataView(buffer); // // console.log(buffer) // // console.log(view) //
console.log('imageForUpload after') // console.log(imageForUpload) // debugger
// // const im = fr.readAsArrayBuffer(imageForUpload.value) // debugger // //
console.log(im) // console.log('e') // console.log(e) //
console.log('cropperRef', cropperRef) // console.log('cropperRef.value',
cropperRef.value) // const { canvas, coordinates, visibleArea } =
cropperRef.value.getResult() // // im = canvas.toDataURL(); // // debugger // //
alert(im) // // console.log(im) // console.log('canvas') //
console.log(canvas.namespaceURI) // console.log(canvas.src) //
console.log(coordinates) // console.dir(visibleArea) // debugger // const ctx =
canvas.getContext('2d'); // console.log(ctx) // ctx.fillStyle = 'green'; //
ctx.fillRect(10, 10, 100, 100); // debugger // const formData = new FormData()
// console.log(canvas) // canvas.toBlob(blob => { //
formData.append('kitchenImage', canvas) // formData.append('userId',
'88jkjklje726') // axios({ // method: 'post', // url:
'http://localhost:4001/kitchens', // data: formData // }) // .then(response => {
// debugger // // let reader = new FileReader() // // reader.onload = () => { //
// imageUpload = reader.result // // } // // reader.readAsDataURL(response.data)
// console.log('imageLoc', response.data.imagePath.toString()) // imageLoc =
response.data.imagePath // }) // .catch (err => { // console.log(err) // }) //
})}
